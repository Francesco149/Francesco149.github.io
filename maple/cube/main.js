...
var wasmBinaryFile;
  wasmBinaryFile = 'main.wasm?ts=1662816879';
...
var ASM_CONSTS = {
  43117: () => { FS.syncfs(function (err) { assert(!err); ccall('storageAfterCommit', 'v'); }); },  
 43196: () => { FS.mkdir('/data'); FS.mount(IDBFS, {}, '/data'); FS.syncfs(true, function (err) { assert(!err); ccall('storageAfterInit', 'v'); }); }
};
...
function deviceScaleFactor() { return js_deviceScaleFactor(); }
function __asyncjs__pyInit(ts) { return Asyncify.handleAsync(async () => { py = await loadPyodide(); await py.loadPackage("numpy"); let zipResponse = await fetch("cubecalc.zip?ts=" + ts); let zipBinary = await zipResponse.arrayBuffer(); py.unpackArchive(zipBinary, "zip"); Module.pyFunc = (x) => py.runPython(` import sys; sys.path.append("cubecalc/"); from glue import ${x}; ${x} `); Module.numCombos = {}; Module.comboLen = {}; Module.matchingLine = {}; Module.matchingProbability = {}; Module.matchingValue = {}; Module.matchingIsPrime = {}; }); }
function pyCalcDebug(enable) { return Module.pyFunc("calc_debug")(enable != 0); }
function pyCalcFree(calcIdx) { return Module.pyFunc("calc_free")(calcIdx); }
function pyCalcSet(calcIdx,key,value) { return Module.pyFunc("calc_set")(calcIdx, key, value); }
function pyCalcWant(calcIdx,key,value) { return Module.pyFunc("calc_want")(calcIdx, key, value); }
function pyCalcWantOp(calcIdx,op,n) { return Module.pyFunc("calc_want_op")(calcIdx, op, n); }
function pyCalcWantPush(calcIdx) { return Module.pyFunc("calc_want_push")(calcIdx); }
function pyCalcWantLen(calcIdx) { return Module.pyFunc("calc_want_len")(calcIdx); }
function pyCalcWantCurrentLen(calcIdx) { return Module.pyFunc("calc_want_current_len")(calcIdx); }
function pyCalc(calcIdx) { return Module.pyFunc("calc")(calcIdx); }
function pyCalcMatchingLen(calcIdx) { return Module.matchingLine[calcIdx].length; }
function pyCalcMatchingComboLen(calcIdx) { return Module.comboLen[calcIdx]; }
function pyCalcMatchingNumCombos(calcIdx) { return BigInt(Module.numCombos[calcIdx]); }
function pyCalcMatchingLoad(calcIdx,maxCombos) { Module.numCombos[calcIdx] = Module.pyFunc("calc_matching_len")(calcIdx); if (Module.numCombos[calcIdx] > maxCombos) { Module.matchingLine[calcIdx] = Module.matchingProbability[calcIdx] = Module.matchingValue[calcIdx] = Module.matchingIsPrime[calcIdx] = []; Module.comboLen[calcIdx] = 0; return; } Module.matchingLine[calcIdx] = Module.pyFunc("calc_matching_lines")(calcIdx); Module.matchingProbability[calcIdx] = Module.pyFunc("calc_matching_probabilities")(calcIdx); Module.matchingValue[calcIdx] = Module.pyFunc("calc_matching_values")(calcIdx); Module.matchingIsPrime[calcIdx] = Module.pyFunc("calc_matching_is_primes")(calcIdx); Module.comboLen[calcIdx] = Module.matchingLine[calcIdx][0].length; }
function pyCalcMatchingLine(calcIdx,i,j) { return BigInt(Module.matchingLine[calcIdx][i][j]); }
function pyCalcMatchingValue(calcIdx,i,j) { return Module.matchingValue[calcIdx][i][j]; }
function pyCalcMatchingProbability(calcIdx,i,j) { return Module.matchingProbability[calcIdx][i][j]; }
function pyCalcMatchingIsPrime(calcIdx,i,j) { return Module.matchingIsPrime[calcIdx][i][j]; }
  ...
  var GLFW = {WindowFromId:function(id) {
        ...
        (function(a1, a2, a3, a4, a5) { throw 'Internal Error! Attempted to invoke wasm function pointer with signature "viiiii", but no such functions have gotten exported!'; })(GLFW.active.id, key, keyCode, status, GLFW.getModBits(GLFW.active));
...
var asmLibraryArg = {
  ...
  "__asyncjs__pyInit": __asyncjs__pyInit,
  ...
  "pyCalc": pyCalc,
  "pyCalcDebug": pyCalcDebug,
  "pyCalcFree": pyCalcFree,
  "pyCalcMatchingComboLen": pyCalcMatchingComboLen,
  "pyCalcMatchingIsPrime": pyCalcMatchingIsPrime,
  "pyCalcMatchingLen": pyCalcMatchingLen,
  "pyCalcMatchingLine": pyCalcMatchingLine,
  "pyCalcMatchingLoad": pyCalcMatchingLoad,
  "pyCalcMatchingNumCombos": pyCalcMatchingNumCombos,
  "pyCalcMatchingProbability": pyCalcMatchingProbability,
  "pyCalcMatchingValue": pyCalcMatchingValue,
  "pyCalcSet": pyCalcSet,
  "pyCalcWant": pyCalcWant,
  "pyCalcWantCurrentLen": pyCalcWantCurrentLen,
  "pyCalcWantLen": pyCalcWantLen,
  "pyCalcWantOp": pyCalcWantOp,
  "pyCalcWantPush": pyCalcWantPush,
...
/** @type {function(...*):?} */
var _storageAfterCommit = Module["_storageAfterCommit"] = createExportWrapper("storageAfterCommit");

/** @type {function(...*):?} */
var _storageAfterInit = Module["_storageAfterInit"] = createExportWrapper("storageAfterInit");
