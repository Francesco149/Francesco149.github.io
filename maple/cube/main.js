...
var wasmBinaryFile;
  wasmBinaryFile = 'main.wasm?ts=1661346971';
...
function __asyncjs__pyInit(ts) { return Asyncify.handleAsync(async () => { py = await loadPyodide(); await py.loadPackage("micropip"); let zipResponse = await fetch("cubecalc.zip?ts=" + ts); let zipBinary = await zipResponse.arrayBuffer(); py.unpackArchive(zipBinary, "zip"); pa = `import sys; sys.path.append("cubecalc/"); `; init = py.runPython(`${pa} from init import init; init`); await init(); Module.pyFunc = (x) => py.runPython(`${pa} from glue import ${x}; ${x}`); }); }
...
function pyCalc(calcIdx) { return Module.pyFunc("calc")(calcIdx); }
  ...
  var GLFW = {WindowFromId:function(id) {
        ...
        if (win.windowCloseFunc)
          (function(a1) { throw 'Internal Error! Attempted to invoke wasm function pointer with signature "vi", but no such functions have gotten exported!'; })(win.id);
